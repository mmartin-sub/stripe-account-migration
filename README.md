...
## Important notice
This fork is implementing the following changes:
- packages have been upgraded. Not every capabilities were tested
- there is no retry policy in case stripe fails or RTM
- during copy prices, currency_options key is deleted for the default currency (not sure if expected)
...
mkdir -p export
# copy products
yarn start copy-products 'export/products.csv' SOURCE_API_KEY TARGET_API_KEY

# copy prices
yarn start copy-prices 'export/products.csv' 'export/prices.csv' SOURCE_API_KEY TARGET_API_KEY
...
yarn start copy-subscriptions 'export/prices.csv' 'export/subscriptions.csv' true SOURCE_API_KEY TARGET_API_KEY
